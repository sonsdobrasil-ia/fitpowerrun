...
export const Route = createFileRoute("/ebooks/$id")({
  ...
  head: () => ({
    meta: [
      { title: "eBook FitPower — página de vendas" },
      {
        name: "description",
        content:
          "Conheça este eBook FitPower: conteúdo, número de páginas, preço e acesso imediato dentro do app.",
      },
      { property: "og:title", content: "eBook FitPower" },
      {
        property: "og:description",
        content: "Detalhes, preço e acesso imediato ao eBook FitPower.",
      },
...
function SalesPage() {
  ...
  const beneficios = [
    ...
    "Atualizações do conteúdo sem custo extra",
  ];

  return (
    <div className="px-5 py-8 max-w-4xl mx-auto">
      <Link to="/" className="inline-flex items-center gap-2 text-sm text-muted-foreground">
        <ArrowLeft className="size-4" /> Estante
      </Link>
      ...
      <div className="mt-6 grid gap-8 md:grid-cols-[280px_1fr] items-start">
        ...
        <div>
          ...
          <div className="mt-8 rounded-2xl border bg-card p-5 shadow-soft">
            <div className="flex items-end justify-between gap-4">
              <div>
                ...
                <p className="text-3xl font-display font-bold text-primary">
                  {formatPreco(book.preco)}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  {book.paginas ?? 0} páginas · acesso imediato
                </p>
              </div>
            </div>
            <Link
            ...
            >
              <ShoppingCart className="size-5" /> Quero este eBook
            </Link>
            <Link
              to="/auth"
              search={{ mode: "login" }}
